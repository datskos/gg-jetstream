use {
    ...
    // jetstreamer patch: debug/info for replay and prune instrumentation.
    ...
    std::{
        ...
        sync::Weak,
        // jetstreamer patch: program-cache prune timing instrumentation.
        time::Instant,
    },
};
...
impl<FG: ForkGraph> ProgramCache<FG> {
    ...
    pub fn prune(
        ...
        // jetstreamer patch: make the size and duration of replay-time cache
        // pruning observable. Agave 4 passes the already-locked fork graph in,
        // so the old retry loop around the internal fork-graph lock is gone.
            ...
            IndexImplementation::V1 { entries, .. } => {
                ...
                for second_level in entries.values_mut() {
                    ...
                    *second_level = second_level
                    ...
                    total_pruned = total_pruned.saturating_add(before_len.saturating_sub(after_len));
                }
                info!(
                    ...
                    if cooperative_loading_task.is_none() {
                        ...
                        match entry {
                            ...
                            Entry::Occupied(mut entry) => {
                                ...
                                debug!(
                                    "program cache cooperative load re-claimed for {:?} (slot {}, owner={:?})",
                                    program_to_load.program_id, slot, owner
                                );
