...
impl Bank {
    ...
    pub fn prune_program_cache(&self, bank_forks: &BankForks) {
        ...
        let mut prep_guard = self
            ...
            .write()
            .unwrap();
        let prep_lock_wait = prep_lock_start.elapsed();
        if prep_lock_wait >= Duration::from_secs(1) {
            warn!(
                "bank prune program cache: waited {:.3}s for epoch boundary lock at slot {}",
                prep_lock_wait.as_secs_f64(),
                self.slot()
            );
        }
        let upcoming_environment = prep_guard.reroot(self.epoch());
        drop(prep_guard);
        ...
        let mut cache_guard = self
            ...
            .write()
            .unwrap();
        let cache_lock_wait = cache_lock_start.elapsed();
        if cache_lock_wait >= Duration::from_secs(1) {
            warn!(
                "bank prune program cache: waited {:.3}s for program cache lock at slot {}",
                cache_lock_wait.as_secs_f64(),
                self.slot()
            );
        }
        ...
        info!(
            "bank prune program cache: slot {} prune took {:.3}s (total {:.3}s)",
            self.slot(),
    ...
    /// [`Bank::prepare_entry_batch`] does, but without taking account locks.
    /// This lets replay sanitize an entry's transactions in parallel and then
    /// lock serially via [`Bank::prepare_sanitized_batch`], preserving account
    /// lock ordering.
    ...
    // jetstreamer patch: public so the replay node can run accounts maintenance.
