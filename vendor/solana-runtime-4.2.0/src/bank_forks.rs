...
impl BankForks {
    ...
    pub fn prune_program_cache(&self, root: Slot) {
        // jetstreamer patch: timing + missing-root diagnostics for replay.
        if let Some(root_bank) = self.banks.get(&root) {
            let start = Instant::now();
            info!(
                "bank forks prune program cache: slot {} epoch {} starting",
                root,
                root_bank.epoch()
            );
            root_bank.prune_program_cache(self);
            info!(
                "bank forks prune program cache: slot {} completed in {:.3}s",
                root,
                start.elapsed().as_secs_f64()
            );
        } else {
            warn!(
                "bank forks prune program cache: slot {} missing root bank",
                root
            );
        }
    }
